...
if required not in loaded.parents:
    raise SystemExit(
        "P11.5a requires the project-local Triton 3.3 overlay; "
        f"loaded {triton.__version__} from {loaded}"
    )
print(f"P11_5A_TRITON_RUNTIME=PASS VERSION={triton.__version__}", flush=True)
PY

exec python titan/scripts/run_p11_probe.py "$@"
